            ...
            if (   (sym_ptr[i].n_type & N_EXT)
                ...
                || (sym_ptr[i].n_type >= N_WEAKU && sym_ptr[i].n_type <= N_WEAKB) )
              {
                ...
                if (big)
                  ...
                  put_16 (address);
#if 1 // 2014-04-20 SHL	don't die if HLL type index too large for OMF
		if (sym_more[i].hll_type <= 0x7fff)
		  {
		    put_idx (sym_more[i].hll_type); /* type index */
		  }
		else {
		  /* Suppress HLL data reference */
		  warning ("HLL debug suppressed for symbol '%.*s' - index %u too large",
			   cchPubName, szPubName, sym_more[i].hll_type);
